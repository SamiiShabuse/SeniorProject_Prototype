...
const mockControls = normalized.map((r, i) => {
  ...
  // Randomly assign DAT and OET statuses
  // 70% chance to randomize even if status exists, 100% if missing
  let datStatus = r.dat?.status
  if (!datStatus || String(datStatus).trim() === '' || rand() < 0.7) {
    datStatus = _datStatuses[Math.floor(rand() * _datStatuses.length)]
  }
  
  let oetStatus = r.oet?.status
  if (!oetStatus || String(oetStatus).trim() === '' || rand() < 0.7) {
